...
| 2026-08-21 | Run #4 chết ngay đầu step "Fetch": `Error: There is a syntax error in .gclient / Line #1, character 37: "solutions = [{ name: src, url: ..., custom_deps: {}, custom_vars: {}, },]"`. Cả 6 retry đều fail theo. | **Regression do fix #3**: viết `.gclient` bằng `gclient config --spec $spec`. Khi PowerShell truyền `$spec` (chứa nháy kép) sang native `gclient.bat`, **nháy kép bị nuốt** → file `.gclient` thành `name: src` (không nháy) → Python parse báo syntax error. File config hỏng nên mọi retry vô nghĩa. Các fix trước (Free-up/path/depot_tools/TLS) vẫn ĐÚNG. | KHÔNG tự viết spec nữa. Dùng lại `fetch --nohooks --no-history chromium` để **nó tự sinh `.gclient` chuẩn** (nháy kép nguyên vẹn), nhưng gọi fetch **đúng 1 lần có canh `Test-Path .gclient`** (tránh lỗi "đã tồn tại" khi re-run) → giữ retry: nếu fetch đứt mạng thì `.gclient` vẫn còn, vòng lặp `gclient sync` 8 lần resume tiếp. Giữ nguyên hardening TLS của fix #3. | ✅ Đã fix (run #5: Fetch XANH ~3h) |
| 2026-08-21 | Run #5: Setup depot_tools ✅ + **Fetch Chromium ✅ (~3h, tải hết cây + đĩa đủ)** nhưng chết ở step "Apply": `error: third_party/blink/renderer/core/frame/navigator_automation_information.cc: No such file or directory ... does not exist in index` → `Patch failed to apply: 0001-hide-navigator-webdriver.patch`. | 5 patch là "best-effort", đường dẫn/anchor **chưa khớp cây v151 thật** (đúng rủi ro đã cảnh báo). Đối chiếu qua googlesource tag 151.0.7922.108: **P01** file `.cc/.h` đã bị xóa, getter dời sang `Navigator::webdriver()` trong `navigator.cc`; **P02** file chromedriver KHÔNG còn (no-op với CDP); **P03** không còn hàm `GetUnmaskedString`, UNMASKED xử lý inline trong `getParameter`; **P04** `getImageData` dời sang `base_rendering_context_2d.cc::getImageDataInternal`; **P05** anchor `is_trusted_(false)` trong `event.cc` vẫn còn (apply được nhưng phản tác dụng). | (a) Viết lại **P01** → `navigator.cc` (replace nguyên thân hàm, tránh dead-code `-Werror`) + **P03** → 2 site UNMASKED trong `getParameter` (guarded software-renderer). (b) BỎ **P02**, HOÃN **P04/P05** → dời `patches/disabled/` (có README lý do). (c) Bước Apply thành **resilient**: continue-on-fail từng patch + in summary + chỉ HARD-FAIL nếu P01 (webdriver) fail hoặc 0 patch apply → drift 1 patch không đốt 3h fetch nữa. Đối chiếu source qua googlesource `+/refs/tags/<tag>/<path>?format=TEXT` (base64), KHÔNG cần clone. | ✅ Đã fix (run #6: Apply XANH, cả 0001+0003 áp được) |
| 2026-08-21 | Run #6 (8b81600): Fetch ✅ + Apply ✅ (0001+0003) → chết ở **Generate GN configuration**: `gn gen` → `ERROR Unresolved dependencies` cho 8 target (browser_tests / interactive_ui_tests / feedback_impl / report_unsafe_site) đều `needs //components/safe_browsing/content/...` (file_type_policies, file_type_policies_test_support, client_side_detection). Ngoài ra log Apply có warning `corrupt patch at line 29/57` (vẫn áp được nhờ `--recount`). | (1) `args.gn` đặt `safe_browsing_mode = 0` → bỏ code safe_browsing khỏi build, nhưng target test/feedback vẫn tham chiếu component đó → `gn gen` (duyệt cả graph, kể cả target test không build) báo unresolved. (2) Header `@@` của 2 patch sai số đếm dòng (0001 ghi 7/9, đúng 8/5; 0003 ghi 10/18, đúng 14/31) → strict apply đầu báo "corrupt patch". | (1) `safe_browsing_mode = 2` để target safe_browsing tồn tại; telemetry vẫn trơ vì `google_api_key` rỗng + wrapper tắt runtime qua prefs. (2) Sửa header `@@` đúng số đếm → áp sạch lần đầu, hết warning. | ✅ Đã fix một phần (run #7: Apply sạch, hết corrupt patch, hết unresolved deps — nhưng mode 2 gây lỗi Android mới, xem dưới) |
| 2026-08-21 | Run #7 (6caac91): Apply ✅ **sạch, không còn warning corrupt patch** (2/2 patch áp qua `--whitespace=fix` ngay lần đầu) + hết "Unresolved dependencies" → vẫn chết ở **Generate GN configuration** nhưng lỗi MỚI: `ERROR at //build/config/android/abi.gni:11:1: Assertion failed. assert(is_android \|\| is_chromeos \|\| is_robolectric)` — chuỗi import: `//components/BUILD.gn:936` → target `//components/safe_browsing/android:native_java_unittests_java` → `rules.gni` → `abi.gni`. | **Chọn sai mode ở fix #6.** Đối chiếu `//components/safe_browsing/buildflags.gni`: mode 0=off, **1=FULL desktop**, **2=mobile/Android** (external API); default là `is_android ? 2 : 1`. Trong `//components/BUILD.gn`, nhánh `else if (safe_browsing_mode == 2)` add đúng 2 target Android (`native_java_unittests_java`, `unit_tests_mobile`) → build Windows đặt mode 2 kéo target Android vào graph → `abi.gni` assert `is_android` fail. Comment fix #6 ghi nhầm "mode 2 = desktop default". | `safe_browsing_mode = 2 → 1` (giá trị desktop đúng theo default `!is_android`). 2 target Android CHỈ nằm dưới nhánh `== 2` nên chuyển sang 1 loại chúng khỏi graph hoàn toàn; các content target (file_type_policies, client_side_detection) mode 1 vẫn compile → hết unresolved. Xác nhận qua source: chỉ 2 dòng `safe_browsing/android` trong file, đều thuộc nhánh mode 2. | ✅ Đã fix (chờ run #8 xác nhận GN→Compile) |
